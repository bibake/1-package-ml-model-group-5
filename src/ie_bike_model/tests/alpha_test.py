...
def load_process_training_data(df):
    """
    ...
    for j in range(0,42):
        for k in range(0,51):
            for l in range(0,101):
                for m in range(0,68):
                    parameters ={"date": dt.datetime(2011, 1, 1, 0, 0, 0),"weathersit": [i],
                                 "temperature_C":[j] ,
                                 "feeling_temperature_C":[k] ,
                                 "humidity":[l] ,
                                 "windspeed":[m] ,}
                    ...
                    try:
                        assert  isinstance(process_new_observation(df), pd.DataFrame) , df
                    except AssertionError as x:
                        error_df=error_df.append(x)

#def test_answer():
  #   assert  isinstance(func(x), pd.DataFrame),

error_df

train_df = load_process_training_data(df)
        df = train_df
